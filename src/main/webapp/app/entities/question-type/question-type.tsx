...
import { getSearchEntities, getEntities } from './question-type.reducer';
...
export class QuestionType extends React.Component<IQuestionTypeProps, IQuestionTypeState> {
  state: IQuestionTypeState = {
    search: ''
  };
  ...
  render() {
    const { questionTypeList, match } = this.props;
    return (
      <div>
        <h2 id="question-type-heading">
          Question Types
          <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
            <FontAwesomeIcon icon="plus" />&nbsp; Create new Question Type
          </Link>
        </h2>
        <Row>
          <Col sm="12">
            ...
            <tbody>
              {questionTypeList.map((questionType, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>
                    {questionType.questionId ? <Link to={`question/${questionType.questionId}`}>{questionType.questionId}</Link> : ''}
                  </td>
