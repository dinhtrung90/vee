...
export const ACTION_TYPES = {
  SEARCH_QUESTIONS: 'question/SEARCH_QUESTIONS',
...
export default (state: QuestionState = initialState, action): QuestionState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_QUESTIONS):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_QUESTIONS):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_QUESTIONS):
...
const apiSearchUrl = 'api/_search/questions';

// Actions

export const getSearchEntities: ICrudSearchAction<IQuestion> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_QUESTIONS,
  payload: axios.get<IQuestion>(`${apiSearchUrl}?query=${query}${sort ? `&page=${page}&size=${size}&sort=${sort}` : ''}`)
});
