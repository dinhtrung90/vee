...
export const ACTION_TYPES = {
  SEARCH_SURVEYS: 'survey/SEARCH_SURVEYS',
...
export default (state: SurveyState = initialState, action): SurveyState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_SURVEYS):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_SURVEYS):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_SURVEYS):
...
const apiSearchUrl = 'api/_search/surveys';

// Actions

export const getSearchEntities: ICrudSearchAction<ISurvey> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_SURVEYS,
  payload: axios.get<ISurvey>(`${apiSearchUrl}?query=${query}${sort ? `&page=${page}&size=${size}&sort=${sort}` : ''}`)
});
...
export const getEntities: ICrudGetAllAction<ISurvey> = (page, size, sort) => {
  ...
  return {
    ...
    payload: axios.get<ISurvey>(`${requestUrl}${sort ? '&' : '?'}cacheBuster=${new Date().getTime()}`)
  };
};
